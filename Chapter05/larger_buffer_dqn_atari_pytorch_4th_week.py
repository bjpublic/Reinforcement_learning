...
# %%
exp_replay = ReplayBuffer(2*10**5)
for i in range(100):
...
for step in range(step, total_steps + 1):
    ...
    if step % eval_freq == 0: 
        Mean_rw_hist=evaluate(
            make_env(clip_rewards=True, seed=step//eval_freq), agent, n_games=3 * n_lives, greedy=True)
            #make_env(clip_rewards=True, seed=step), agent, n_games=3 * n_lives, greedy=True)
        
        # initial_state_q_values = agent.get_qvalues([make_env(seed=step).reset()])
        initial_state_q_values = agent.get_qvalues([make_env(seed=step//eval_freq).reset()])
    ...
    if Mean_rw_hist > RW_record and (step) % 1e+04 == 0:
        RW_record = Mean_rw_hist
        print('Check point save')
        torch.save(agent,f'./ckpt/2e05/batch_{batch_size}_randomseed_{seed}_ckpt_{step}_Atari_model.pth')

    if Mean_rw_hist >= 100:
        print('---------------------------------')
        print('Now, agent palys well as much as Human level!')
        print('Train process finished!')
        torch.save(agent,f'./ckpt/2e05/batch_{batch_size}_seed_{seed}_ckpt_{step}_Atari_model.pth')
        break
...
print('Cool!')
print('---------------------------')
print('All train process finished!')
torch.save(agent,f'./ckpt/2e05/batch_{batch_size}_seed_{seed}_fin_episode_Atari_model.pth')
